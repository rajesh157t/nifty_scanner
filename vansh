...
def get_fno_stocks():
    try:
        ...
        return ["RELIANCE","TCS","INFY","HDFCBANK","ICICIBANK","SBIN","BHARTIARTL","LT","ITC","TATAMOTORS","AXISBANK","KOTAKBANK","BAJFINANCE","MARUTI","WIPRO"]
...
if not (14 <= vix_val <= 20):
    print(f"VIX {round(vix_val,2)} out of range")
else:
    ...
    for symbol in SCAN_LIST:
        ...
        for item in chain['records']['data']:
            ...
            all_signals.append({
                "symbol": symbol, "strike": k, "spot": spot,
                "buy": buy, "tgt": tgt, "sl": sl,
                "delta": round(delta,2), "tv": round(tv_pct,1),
                "oi": oi_pct, "vol": vol, "score": score, "vix": round(vix_val,1)
            })
            break
    ...
    if not all_signals:
        ...
        print(f"Total Found: {len(all_signals)} -> Sending Top {len(top2)}")

        for s in top2:
            tag = "INDEX" if s['symbol'] in ["NIFTY","BANKNIFTY"] else "STOCK"
            text = (
                f"🔥 *TOP {top2.index(s)+1} | {tag}: {s['symbol']} {s['strike']} CE*\n"
                f"Spot:{s['spot']} | VIX:{s['vix']} | Score:{int(s['score'])}\n"
                f"BUY:`{s['buy']}` (15M ATR)\n"
                f"TGT:`{s['tgt']}` (1H ATR)\n"
                f"SL:`{s['sl']}`\n"
                f"Δ:{s['delta']} TV:{s['tv']}% OI:{s['oi']}% Vol:{s['vol']}"
            )
            send_telegram(text)
